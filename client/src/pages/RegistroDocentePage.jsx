  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${API_URL}/api/public/registro-docente`,
        form,
      );
  ...
  if (credenciales) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-lg w-full text-center border-t-8 border-blue-600 animate-in fade-in zoom-in duration-300">
          <div className="mx-auto bg-blue-100 w-16 h-16 rounded-full flex items-center justify-center mb-4">
            <CheckCircle size={32} className="text-blue-600" />
          </div>
          <h2 className="text-2xl font-bold text-gray-800 mb-2">
            ¡Registro Docente Exitoso!
          </h2>
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden">
        <div className="bg-[#1e3a8a] p-6 text-white text-center">
          <h1 className="text-2xl font-bold uppercase tracking-wide flex justify-center items-center gap-2">
            <Briefcase /> Registro de Docentes
          </h1>
          <p className="text-white/80 text-sm mt-1">
            Completa tus datos para generar tu clave y correo institucional
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
              <label className="block text-xs font-bold text-blue-800 uppercase mb-1 flex items-center gap-1">
                <Mail size={14} /> Correo Personal (Contacto)
              </label>
              <input
                required
                type="email"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="label-form">Sede Principal *</label>
                <select
                  required
                  name="sede_id"
                  value={form.sede_id}
            ...
            <button
              disabled={cargando}
              type="submit"
              className="w-full bg-[#1e3a8a] hover:bg-[#1e40af] text-white py-4 rounded-xl font-bold text-lg shadow-xl mt-6 transition transform hover:scale-[1.01]"
            >
              {cargando ? "Procesando..." : "FINALIZAR REGISTRO DOCENTE"}
            </button>
          </form>
        </div>
      </div>
      <style>{`
        ...
        .input-form:focus { ring: 2px; ring-color: #1e3a8a; border-color: #1e3a8a; background-color: #fff; box-shadow: 0 0 0 4px rgba(30, 58, 138, 0.1); }
      `}</style>
    </div>
  );
};
